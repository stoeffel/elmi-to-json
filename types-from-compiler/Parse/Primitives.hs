...
-- PARSER
newtype Parser x a =
  Parser (forall b. State -> (a -> State -> b) -- consumed ok
                              -> (a -> State -> b) -- empty ok
...
oneOfHelp state cok eok cerr eerr toError parsers =
  case parsers of
    Parser parser:parsers ->
      let eerr' _ _ _ = oneOfHelp state cok eok cerr eerr toError parsers
      in parser state cok eok cerr eerr'
    [] ->
...
oneOfWithFallback parsers fallback =
  Parser $ \state cok eok cerr _ -> oowfHelp state cok eok cerr parsers fallback
...
instance Monad (Parser x) where
  {-# INLINE return #-}
  return value = Parser $ \state _ eok _ _ -> eok value state
  {-# INLINE (>>=) #-}
...
toErr row col toError = Left (toError row col)
...
getCol = Parser $ \state@(State _ _ _ _ _ col) _ eok _ _ -> eok col state
...
addLocation (Parser parser) =
  Parser $ \state@(State _ _ _ _ sr sc) cok eok cerr eerr ->
    let cok' a s@(State _ _ _ _ er ec) =
          cok (A.At (A.Region (A.Position sr sc) (A.Position er ec)) a) s
        eok' a s@(State _ _ _ _ er ec) =
...
getIndent =
  Parser $ \state@(State _ _ _ indent _ _) _ eok _ _ -> eok indent state
...
-- CONTEXT
inContext ::
     (x -> Row -> Col -> y) -> Parser y start -> Parser x a -> Parser y a
...
word1 word toError =
  Parser $ \(State src pos end indent row col) cok _ _ eerr ->
    if pos < end && unsafeIndex pos == word
      then let !newState = State src (plusPtr pos 1) end indent row (col + 1)
           in cok () newState
...
word2 w1 w2 toError =
  Parser $ \(State src pos end indent row col) cok _ _ eerr ->
    ...
    in if pos1 < end && unsafeIndex pos == w1 && unsafeIndex pos1 == w2
         then let !newState = State src (plusPtr pos 2) end indent row (col + 2)
              in cok () newState
         else eerr row col toError
...
isWord pos end word = pos < end && unsafeIndex pos == word
